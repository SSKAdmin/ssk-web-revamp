...
async function createMinimalBanner() {
  const width = 1584;
  const height = 396;
  ...
  // Linear gradient for a premium, non-exaggerated dark blue background
  logoPaths += "<linearGradient id='bgGrad' x1='0%' y1='0%' x2='100%' y2='100%'>";
  logoPaths += "<stop offset='0%' stop-color='#080C14' />";
  logoPaths += "<stop offset='100%' stop-color='#121C2D' />";
  logoPaths += "</linearGradient>";
  logoPaths += "</defs>";
  
  // Solid clean background
  logoPaths += "<rect width='1584' height='396' fill='url(#bgGrad)' />";

  // SSK Logo Container centered horizontally on the left side
  logoPaths += "<g transform='translate(150, 100) scale(1.1)'>";
  ...
  // Clean, Unexaggerated Typography (Centered on the right side)
  logoPaths += "<g transform='translate(680, 200)'>";
  logoPaths += "<text x='0' y='0' font-family='-apple-system, BlinkMacSystemFont, Arial, sans-serif' font-weight='500' font-size='42' fill='#FFFFFF' letter-spacing='1'>SSK CONSULTING</text>";
  logoPaths += "<line x1='0' y1='25' x2='40' y2='25' stroke='#1d9cf0' stroke-width='3' />";
  logoPaths += "<text x='0' y='60' font-family='-apple-system, BlinkMacSystemFont, Arial, sans-serif' font-weight='300' font-size='20' fill='#A0ABB8' letter-spacing='0.5'>Digital Transformation &amp; IT Strategy</text>";
  logoPaths += "</g>";

  const svgStr = "<svg width='1584' height='396' xmlns='http://www.w3.org/2000/svg'>" + logoPaths + "</svg>";

  const outPath = './public/SSK_Minimal_Banner.png';
